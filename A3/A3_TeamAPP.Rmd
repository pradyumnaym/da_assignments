...
<h4>Ans. We typically use regularization when our model has fit the training data too well and fails to generalize, and doesnt perform well on test set. In such cases, the bias is Low , but the variance is high. In oder to fix this, we introduce some bias to decrease the variance of the model. THis way, the model performs better on the test set as well. </h4>
...
<br></h4>Ans. The OLS algorithm for linear regression has no bias but can suffer from a large variance. Both ridge and lasso regression introduce some bias to reduce this variance. Both of these are regularised linear regression models. However, there are a few key differences between them.
<ul>
